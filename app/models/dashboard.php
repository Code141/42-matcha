...
class m_dashboard
{
	...
	{
		$sql = "
			SELECT DISTINCT u.id, u.username, u.id_media, c.timestamp as last_connexion, conv.id as conv_id
			FROM `like` l1
			LEFT JOIN `like` l2
			...
			ON ((conv.id_user_from = :id_user AND conv.id_user_to = u.id)
			OR (conv.id_user_from = u.id AND conv.id_user_to = :id_user))
